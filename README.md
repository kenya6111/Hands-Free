...
- Google Cloud Speech-to-Text(Google音声認識API)
  ...
  　- 検査A（日常点検）
    ...
    - 懸念点:
        - 音声データをクラウドに送信するためネットワーク接続が必須
        - 作業人数によっては人数ランニングコストが現実的でなくなる
    - 1人〜10人規模 | 月1万円〜数万円程度 → 会社的にほぼ気にならないコスト。現実的。
    - 50人以上（ライン全体） | 月7万〜15万超え → 継続利用なら会社内で**「本当にコスパいいか？」議論になるレベル**
    - 100人以上大規模展開 | 年間数百万円 → オンプレ（自社運用）を検討すべき金額帯に突入する
    - 「それだけのコストをかけて、どれだけ工数削減できるの？」
    - 「何人分の労働時間を削減できるの？」
    - みたいな効果説明をしないと、上が納得しなくなる。
    - この辺りの算出が必要みたいだねAPI使う場合は
- Microsoft Azure Speech Services
    - 従量課金制
        - $1.00 / 時間（≒ 約0.0166ドル/分）	1秒単位課金
    - 定額プラン
        -　$1,600 for 2,000 hours	$0.80 per hour
        - 人数 × 10時間 × $1 = $1,600
            → 人数 = 160人
        - 月の音声入力時間が10時間の前提であれば、作業員が160人以上の場合に使うべき
- Amazon Transcribe
    - https://aws.amazon.com/transcribe/pricing/
    - 標準音声認識（バッチ・リアルタイム）
    - $0.024/分	$1.44/時間
    - 1分あたり**$0.024**（GoogleやAzureよりやや高い）
- PCスペック
    - M3チップ搭載
...
- VRAMとは何か
    - VRAM (Video Random Access Memory) は、グラフィックスカード（GPU）が利用するメモリ領域のことです。
    - GPU上でニューラルネットワークを動作させる際、モデルのパラメータ（重み）や中間テンソルなどを格納するためにVRAMを消費します。
    - 大規模なAIモデルほど多くのVRAMが必要で、足りない場合は推論中にエラーが出たり、速度が低下したりする原因となります。
- RTXとは何か
    - NVIDIA RTX は、NVIDIA社のGPU製品ラインの名称です。
    - 例: GeForce RTX 3060, 3080, 3090, 4090 など（コンシューマ向け）
    - 企業向けだとTITAN RTX, A100, L40などのデータセンター用製品も存在。
    - RTXシリーズは「Ray Tracing eXtreme」の略称とされ、Turing/ Ampere/ Ada Lovelaceなどのアーキテクチャで構成されている。
    - ディープラーニングやCUDA計算に対応しており、ニューラルネットワークの高速処理（行列演算）に特化したハードウェア（CUDAコア、Tensorコア）を備えています。
    - Whisperのような深層学習モデルを推論する際、GPUの性能が高いほど短時間で処理が終わるため、このRTXシリーズがよく活用されます。
- バッファって何？
    - 「ある程度まとめてから処理する」こと。リアルタイムじゃなく、ちょっと溜めてから考える。
    - たとえば：
    - あなたが「こんにちは」って喋るとする
    - Whisperは「こ」「ん」「に」...ってすぐに処理しない
    - 最後まで聞いて「こんにちは」で1つの文として処理する
    - つまり：
    - “30秒の音声を聞いてから処理する”＝バッファ30秒
- Google Colaboratoryとは？
    - Googleアカウントを作成するだけでPythonを活用したプログラム開発を行えるサービスです。機械学習やディープラーニングに役立つ豊富なライブラリも用意されており、無料でGPUを使える点も強み
    ・ローカル環境の構築不要
    ・必要なフレームワークやライブラリを追加可能
    ・Pythonコードの共有が容易
    ・無料でGPUを利用可能
    ・Python開発に最適化された対話型プログラミング環境
- 文系でも\*\*「GPUの種類・性能・違い」\*\*が深く理解できるように、たとえ話＋図式で丁寧に解説するね。
```txt
🧠 GPUってなに？
GPUは、**大量の計算をめちゃくちゃ速くできる計算マシン**のこと。
本来はゲーム・映像処理用だけど、今はAIの脳みそとしても大活躍中。

🚗 GPUを「車」にたとえると？
AIの計算処理は「山道を走って荷物を届ける」ようなもの。
GPUの種類によって、その\*\*スピード・パワー・積載量（メモリ）\*\*が違う。

🔰 代表的なGPUのたとえ
| GPU名         | イメージ         | 車のたとえ         | 特徴・用途（ざっくり）                 |
| ------------ | ------------ | ------------- | --------------------------- |
| **T4 GPU**   | 軽自動車＋ターボ     | 安定して速い宅配車     | Whisperや画像AIに最適。Colabで無料も可能 |
| **L4 GPU**   | ハイブリッドスポーツカー | 速くて省エネ        | T4より速く省エネ。今後の主力             |
| **A100 GPU** | F1レーサー       | 超高速・燃費悪い      | 超大規模AI（ChatGPT級）に必要         |
| **V100 GPU** | 大型トラック       | 大量の荷物を超安定で運ぶ  | 研究や学習用に今も現役                 |
| **TPU**      | ロボット専用工場     | AI専用に作られた自動機械 | Googleが作ったAI専用チップ           |

🔍 性能ってどう違うの？
たとえば Whisper（音声認識AI） を動かす場合：
| GPU  | 30秒の音声を処理する時間 | Whisper動作 |
| ---- | ------------- | --------- |
| CPU  | 40秒以上かかる      | 遅すぎて実用外   |
| T4   | 約10秒          | 十分使える     |
| L4   | 約5〜7秒         | 快適でおすすめ   |
| A100 | 2秒以下          | 爆速だけど高価   |

💸 値段も違う
| GPU  | 1時間あたりの料金（Colab Pro例） |
| ---- | --------------------- |
| T4   | 0円（無料プランでも使える）        |
| L4   | 有料（プレミアムのみ）           |
| A100 | 高額（商用プランのみ）           |
...
| あなたの目的                     | 使うGPU       |
| -------------------------- | ----------- |
| 音声認識や画像処理を無料で試したい          | T4          |
| 処理をもっと速くしたい＋お金出せる          | L4          |
| 大規模AI学習・ChatGPTみたいなモデル作りたい | A100        |
| GoogleのAIだけ使う              | TPU（特別なケース） |

✅ 超まとめ
GPUは「AIの計算をするためのエンジン」。
種類が違えば**速度・コスト・使い道**が全然違う。
> 文系的に言えば「車のグレード違い」。あなたが乗りたい使い方に合ったものを選ぶだけ。
---
```
- 以下では、**Google Colab上でWhisperをGPUで動かし、処理速度を測定するまでの一連の手順**を、できるだけ詳しく説明します。
 ```txt
    ## 1. Googleアカウントの準備
    1. Googleアカウントをお持ちでない場合は、まず[https://accounts.google.com/](https://accounts.google.com/) から作成してください。
    2. ログインしておけば、Colabへアクセスすると自動で認証されるようになります。
    
    
    ## 2. Google Colabにアクセスし、新しいノートブックを作成
    1. ブラウザで [https://colab.research.google.com/](https://colab.research.google.com/) を開く。
    2. 「+ 新しいノートブック」または「New Notebook」ボタンをクリック。もしくは「File（ファイル）」→「New notebook（新しいノートブック）」でもOK。
    3. ノートブックが作成されると、「Untitled0.ipynb」などのファイル名が表示され、Pythonコードを記述できるセルが1つ初期表示されます。        


    ## 3. ランタイムのハードウェア アクセラレータをGPUに変更
    Colabは初期設定で「None（CPUのみ）」を使う場合があります。GPUを有効にするには以下の操作が必要です。
    1. 上部メニューの「ランタイム」→「変更」 (英語UIだと「Runtime」→「Change runtime type」)。
    2. 「ハードウェア アクセラレータ (Hardware accelerator)」を **`GPU`** に指定。
       * 「None」や「TPU」ではなく「GPU」を選択。
    3. 「保存」ボタンを押す。
    4. これでノートブックのランタイムがGPU対応となり、新しくバックエンドセッションが作り直されます。


    ## 4. Whisper用ライブラリをインストール
    Colabの環境は再起動（またはセッション切断）されると初期化されるため、必要なPythonライブラリは**毎回インストールする**のが一般的です。    
    1. ノートブック最初のコードセルに以下のコマンドを書き込む:
        ```python
        !pip install git+https://github.com/guillaumekln/faster-whisper.git
        ```
        Whisperの公式実装（`openai-whisper`）より高速化された[faster-whisper](https://github.com/guillaumekln/faster-whisper)を例にして説明します。
        あるいは、公式版が良ければ `!pip install openai-whisper` でOK。
    2. そのセルを実行すると、Colabのコンソール画面に`pip`のログが表示され、しばらくするとインストール完了のメッセージが出ます。
    3. PyTorchなどGPU対応ライブラリはColabにプリインストール済みですが、`faster-whisper`が内部でCTranslate2などをビルドするので多少時間がかかる場合があります。


    ## 5. Whisper動作テストと推論速度の計測    
    ### (A) サンプルコードの入力
    1. 新しいコードセルを追加する（「+ コード」ボタン）し、以下の例を入力してみてください:
    ...
    # 大きなモデルほど精度が上がるがメモリや速度が犠牲になる。
    # small, medium, large-v2 などを試してみる
    MODEL_SIZE = "small"
    
    print("Loading model:", MODEL_SIZE)
    # device="cuda" を指定しGPUを使う
    model = WhisperModel(MODEL_SIZE, device="cuda")
    
    # 適当な短いサンプル音声ファイルをダウンロード or アップロードして wav_data にする
    # 例としてGoogle Driveにある audio.wav を読み込む想定など
    # ここでは仮に wav_data として numpy float配列があるとする
    # 実際には実データを読み込んで代入してください
    import numpy as np
    sample_rate = 16000
    duration_sec = 2
    # ダミーで無音を作成。実際は音声を読み込んで割り当てる
    wav_data = np.zeros(int(sample_rate * duration_sec), dtype=np.float32)
    
    # Warm-up: 1回目の推論はロードやキャッシュで時間がかかる
    _ = model.transcribe(wav_data, language="ja")
    
    # 実際の推論を計測
    start = time.time()
    segments, info = model.transcribe(wav_data, language="ja")
    end = time.time()
    ...
    - ここでは仮に**無音のnumpy配列**を作って推論しているため、実際の結果は空白に近いテキストになります。
       * 実際はWAVファイルを読み込んで `wav_data` に割り当てる形に変えてください（`soundfile` や `librosa` を使ってファイルを読み込み→numpyに変換）。
       * 推論時間を計測することで、大まかな速度を知ることができます。
    
    ### (B) 音声ファイルをアップロードして試す方法
    
    1. Colab左の「ファイル」タブから「ファイルをアップロード」して、`audio.wav`のようなファイルを配置。
    2. 以下のように音声を読み込み:
    
    ```python
    import soundfile as sf
    
    wav_data, sr = sf.read("/content/audio.wav")
    # Whisperはfloat32, sample_rate=16000が推奨なので変換
    import numpy as np
    wav_data = wav_data.astype(np.float32)
    # サンプリングレートが16000でない場合はリサンプリング
    if sr != 16000:
        import librosa
        wav_data = librosa.resample(wav_data, orig_sr=sr, target_sr=16000)
        sr = 16000
    
    # 以降は上記の model.transcribe(...) と同様に処理する
    ```
    
    3. この手順で推論を行い、**`time.time()`などで前後を計測**すれば実行時間を確認できます。
    
    ---
    
    ## 6. 実行結果の確認
    
    * 実行すると、コンソール上に「Inference time = X.XX sec」と表示されます。
    * 例えば2秒の音声を0.5秒で処理できたなら、**RTF** = 0.25（実時間より速い）。
    * モデルを `medium` に変えたり、 `large-v2` に変えてみたりして速度変化を観察できます。
    * GPUが割り当てられていないと**CPUモード**で動き、何十秒かかる可能性もあるので注意。
    
    ---
    
    ## 7. 連続使用やセッション切断に関する注意
    
    * Colabは無料枠だと、**一定時間（数時間）使用するとセッションがリセットされる**ことがあります。
    * また、頻繁に利用していると「GPUが割り当てられない」状態になる場合があります。
    * 長時間動かしたり大量推論を行いたい場合は有料版のColab Pro / Pro+を検討すると延長できますが、それでも本格的な商用運用には向きません。
    
    ---
    
    ## 8. まとめ
    
    1. **GoogleアカウントでColabにアクセス**
    2. **新しいノートブックを作成 → ランタイムをGPUに変更**
    3. **Whisperライブラリをpipでインストール**
    4. **サンプルコードを実行し、推論速度をtime.time()等で測定**
    5. **音声ファイルをアップロードして試す場合**は`soundfile`等で読み込むかDrive連携してロード
    
    これらの操作で、**無料のGPU環境**を簡単に試すことができ、Whisperの速度測定や動作確認が行えます。
  ```
...
## ■プロジェクト概要  
東海地区のトヨタ系列製造業者向け展示会にて、自社プロダクトの音声認識機能を搭載したプロトタイプを企画・開発。
車両の採寸工程における「採寸作業」と「帳票記録」の往復作業を、ヘッドセットによる音声入力のみで完結させることで、作業効率と安全性の向上を目指す取り組み。
展示会では多くの来場者から高い関心を得ることに成功し、後日トヨタ子会社から正式な開発依頼を獲得（経営層判断により最終的には中止）。
...
### ① 実運用に最適な音声認識技術の選定  
**課題**  
音声入力機能を実装するにあたり、精度・速度・導入コストのバランスが取れたSTT（Speech to Text）技術を選定する必要があった。既存のChrome標準APIは精度は高いが利用制限があり、代替技術の調査が求められた。

**アプローチ**  
VOSK、Google Web Speech API、その他オープンソースSTTエンジンを対象に精度・処理速度・モデルサイズ・実装容易性を比較検証
Pythonベースでの動作テストを実施し、推論速度や誤認識率を定量的に計測
モデルサイズや推論レイテンシから、実運用可能性とユーザ体験を踏まえて選定

**成果**  
実運用環境において最もバランスの取れたSTTエンジンを選定
精度要件を満たしつつ、モデルサイズを1押さえた構成を確立

---

### ② 音声モデルの誤認識率改善
**課題**  
音声認識モデルの選定後、特定の業務用語や固有名詞に対して誤認識が頻発し、業務効率やユーザー体験の低下が懸念された

**アプローチ**  
認識ログを分析し、誤認識が多発する単語・文脈を特定
そもそもの業務で用いられる用語の調査
 事前プロンプトの活用

**成果**  
実運用環境において最も精度・速度バランスの取れたSTTエンジンを選定
精度要件を満たしつつ、モデルサイズを抑えた構成を確立
